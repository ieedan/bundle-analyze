...
export function displaySize(bytes: number): string {
  ...
  function getSizeString(bytes: number) {
    if (bytes < KILOBYTE) {
      return `${displayDividedSize(bytes)} B`;
    }

    if (bytes < MEGABYTE) {
      return `${displayDividedSize(bytes / KILOBYTE)} KB`;
    }

    if (bytes < GIGABYTE) {
      return `${displayDividedSize(bytes / MEGABYTE)} MB`;
    }
  ...
  if (showRed) {
    return pc.red(getSizeString(bytes));
  }
  if (showYellow) {
    return pc.yellow(getSizeString(bytes));
  }
